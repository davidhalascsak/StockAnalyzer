...
struct FinancialView: View {
    ...
    var incomeStatementView: some View {
        VStack(alignment: .leading) {
    ...
    var balanceSheetView: some View {
        VStack(alignment: .leading) {
            ...
            .cornerRadius(10)
            BarChartView(title: "Share Outstanding", xData: vm.incomeStatement.map({$0.date}).reversed(), yData: vm.incomeStatement.map({$0.shareOutstanding}).reversed(), intervals: [1,3,5,10], isInverted: true, reversePrefix: false)
        }
    }
    
    var cashFlowStatementView: some View {
        VStack(alignment: .leading) {
            Text("Cash Flow")
                .font(.title2)
                .fontWeight(.bold)
